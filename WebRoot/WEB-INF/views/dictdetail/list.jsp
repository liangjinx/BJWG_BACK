...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		//搜索
		function search(){
			var url = "<common:WebRoot/>/v/dictDetail/list?";
 ...
 * 资源编辑
 */
function dictDetailEidt(type,id){
	var action = "";
	switch (type) {
	case 1://新增
		action = basePath + "/v/dictDetail/editInit";
		break;
	case 2://修改
		action = basePath + "/v/dictDetail/editInit";
		$("#id").val(id);
		break;
	case 3://单个删除
		action = basePath + "/v/dictDetail/delete";
		if(confirm("是否确定删除记录？")){
			$("#id").val(id);
		}else{
			return ;
...
<div class="admin">
      <div class="tab-body">
        ...
        <div class="tab-panel active" id="tab-set">
        	...
        	<form action="<common:WebRoot/>/v/dictDetail/list" method="post" id="allProjectForm" name="allProjectForm">
        	
        		<%-- 隐藏域 --%>
        		<input type="hidden" id="id" name="id" value="">
        		
			    <div class="panel admin-panel">
			    	<div class="panel-head"><strong>字典明细配置列表</strong></div>
			        <div class="padding border-bottom">
			            ...
			            <strong style="padding-left: 100px;">
				       		...
				       		<input type="button" class="button button-small border-blue" value="搜索" onclick="search();"/>&nbsp;
				            	<input type="button" class="button button-small border-green" value="新增字典明细项" onclick="dictDetailEidt(1)"/>
				            	<input type="button" class="button button-small border-yellow" value="批量删除" onclick="dictDetailEidt(4)"/>
			            </strong>
			        ...
			        <table class="table table-hover">
			        	...
			        	<c:forEach items="${requestScope.list }" var="l">
				            <tr>
				            	<td><input type="checkbox" name="idList" id="idList" value="${l.dictDetailId}" /></td>
				            	<td>
				            		${l.dictId}
				            	</td>
				            	<td>${l.name }</td>
				            	...
				            	<td>${l.value }</td>
				            	<td>
				            		<a class="button border-blue button-little" href="#" onclick="dictDetailEidt(2,${l.dictDetailId});return false;">编辑</a>
				            		<a class="button border-yellow button-little" href="#" onclick="dictDetailEidt(3,${l.dictDetailId});return false;">删除</a>
				            	</td>
				            </tr>
			        	</c:forEach>
